...
**多 Agent 协作 · 本地零成本可跑 · 回复里的数字全部由 SQL 算出**
...
- 可选：[Ollama](https://ollama.com/) + 本地小模型（默认 `qwen-bill-1.5-1.8b-q4km`）
- 可选：任一 OpenAI 兼容 API——**不配也能跑通全流程**
...
默认走本地模型。想让表达更自然，可配置外部 API（OpenAI 兼容协议，此处以智谱为例）：
...
也可写进项目根 `.env`（已集成 `load_dotenv()`）；注意 `.env` 优先级低于已存在的环境变量，二者择一。`.env` 含密钥，**切勿提交**。
...
一句话概括：**模型负责把话说好听，数字不归它管**。这也是为什么本项目敢在财务场景里用 1.8B 的本地小模型。
...
| 不能依赖外部服务 | 本地模型默认通道优先，外部模型可选增强，断了也不影响主流程 |
...
flowchart TB
    ...
    EXT["Ollama（本地模型）/ 可选外部 LLM API<br/>SQLite / FAISS / 个人笔记库"]
...
| 模型接入 | 自研 ChatModel harness | 能力契约 + 路由 + 参数归一 + 结构化错误；本地 Ollama / OpenAI 兼容双通道 |
...
- **子 Agent 用协程不用进程**：主进程不做重计算（推理在 Ollama / 云端），协程足以覆盖 IO 等待；进程化反而带来 IPC、状态序列化与数倍内存开销。
...
- **本地小模型能力有限**：复杂长句的稳定性不如外部模型，这也是保留外部通道与本地确定性渲染兜底的原因。
